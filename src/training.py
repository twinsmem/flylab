...
class Trainer:
    ...
    # ---------- 众包测试 ----------
    def crowd_test(self, img256):
        """访客手绘/上传图像 → 蝇的读出（Top3 候选）。"""
        classes = [int(c) for c in LEVEL_CLASSES[self.level]]
        rng = np.random.default_rng(int(time.time() * 1e6) % 2**31)
        with self.lock:
            kc, mi = self.brain.present(img256, rng)
            vals = {c: mi[c * self.brain.mpc:(c + 1) * self.brain.mpc].sum()
                    for c in classes}
        order = sorted(vals, key=vals.get)
        rec = {
            "kind": "crowd", "t": self.brain.t_present,
            "pred": str(order[0]), "top3": [str(c) for c in order[:3]],
            "img": img_to_b64(img256),
        }
        self.crowd_log.append(rec)
        self.crowd_log = self.crowd_log[-200:]
